...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    console.log('4. Extracted fields:', { username, email, hasPassword: !!password });
...
// @desc    Register new user (alias for signup)
// @access  Public
router.post('/register', async (req, res) => {
  // Exact same logic as signup
  try {
    ...
    const user = new User({
      ...
      firstName: firstName || '',
      lastName: lastName || '',
      avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(username || email)}&background=random`,
      ipAddress: req.ip || req.connection.remoteAddress
    });
